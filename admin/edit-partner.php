...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($name)) {
        ...
        // Initialize logo path with existing value
        ...
        if (empty($error_message)) {
            $sql = "UPDATE partners SET name = ?, logo_path = ?, website_url = ?, display_order = ?, is_active = ? WHERE id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("sssiis", $name, $logo_path, $website_url, $display_order, $is_active, $id);
...
<div class="content-card">
    ...
    <form action="" method="POST" enctype="multipart/form-data">
        <div style="margin-bottom: 20px;">
            ...
            <input type="text" id="name" name="name" required style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px; box-sizing: border-box;" value="<?php echo htmlspecialchars($partner['name']); ?>">
        </div>
        ...
        <div style="margin-bottom: 20px;">
            ...
            <?php if (!empty($partner['logo_path'])): ?>
                <div style="margin-bottom: 10px;">
                    <img src="<?php echo '../' . $partner['logo_path']; ?>" alt="Current logo" style="width: 150px; height: 150px; object-fit: contain; border: 1px solid #ddd; padding: 10px; border-radius: 5px;">
                    <p>Current logo</p>
                </div>
            <?php endif; ?>
            <input type="file" id="logo" name="logo" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px; box-sizing: border-box;">
            <small style="color: #666; display: block; margin-top: 5px;">Leave empty to keep current logo. Recommended size: 150x150 pixels. Max file size: 2MB. Supported formats: JPG, PNG, WEBP</small>
        </div>
        
        <div style="margin-bottom: 20px;">
            <label for="website_url" style="display: block; margin-bottom: 8px; font-weight: bold;">Website URL</label>
            <input type="url" id="website_url" name="website_url" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px; box-sizing: border-box;" value="<?php echo htmlspecialchars($partner['website_url']); ?>">
        ...
        <div style="margin-bottom: 20px;">
            ...
            <input type="number" id="display_order" name="display_order" min="0" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px; box-sizing: border-box;" value="<?php echo $partner['display_order']; ?>">
        ...
        <div style="margin-bottom: 20px;">
            <label style="display: flex; align-items: center; cursor: pointer;">
                <input type="checkbox" name="is_active" value="1" <?php echo $partner['is_active'] ? 'checked' : ''; ?> style="margin-right: 10px;">
        ...
        <div style="display: flex; gap: 10px;">
            <button type="submit" style="background: #48825d; color: white; border: none; border-radius: 5px; padding: 12px 20px; cursor: pointer; transition: all 0.3s ease;">
                <i class="fas fa-save" style="margin-right: 5px;"></i> Update Partner
            </button>
