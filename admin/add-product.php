...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($name)) {
        ...
        if (empty($error_message)) {
            $sql = "INSERT INTO products (name, description, price_display, product_url, image_path, location, display_order, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssssssis", $name, $description, $price_display, $product_url, $image_path, $location, $display_order, $is_active);
            
            if ($stmt->execute()) {
                $success_message = "Product added successfully!";
                // Reset form data
                $name = $description = $price_display = $product_url = $location = '';
                $display_order = 0;
                $is_active = 1;
            } else {
...
<div class="content-card">
    ...
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        <div style="margin-bottom: 20px;">
            ...
            <input type="text" id="description" name="description" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px; box-sizing: border-box;" value="<?php echo isset($description) ? htmlspecialchars($description) : ''; ?>">
        ...
        <div style="margin-bottom: 20px;">
            ...
            <input type="text" id="price_display" name="price_display" required style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px; box-sizing: border-box;" value="<?php echo isset($price_display) ? htmlspecialchars($price_display) : ''; ?>">
        ...
        <div style="margin-bottom: 20px;">
            ...
            <input type="url" id="product_url" name="product_url" required style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px; box-sizing: border-box;" value="<?php echo isset($product_url) ? htmlspecialchars($product_url) : ''; ?>">
        ...
        <div style="margin-bottom: 20px;">
            ...
            <small style="color: #666; display: block; margin-top: 5px;">Recommended size: 500x500 pixels. Max file size: 2MB. Supported formats: JPG, PNG, WEBP</small>
        </div>
        
        <div style="margin-bottom: 20px;">
            <label for="location" style="display: block; margin-bottom: 8px; font-weight: bold;">Location</label>
            <input type="text" id="location" name="location" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px; box-sizing: border-box;" value="<?php echo isset($location) ? htmlspecialchars($location) : 'Bagmati Province'; ?>">
        </div>
        ...
        <div style="display: flex; gap: 10px;">
            <button type="submit" style="background: #48825d; color: white; border: none; border-radius: 5px; padding: 12px 20px; cursor: pointer; transition: all 0.3s ease;">
                <i class="fas fa-save" style="margin-right: 5px;"></i> Save Product
            </button>
